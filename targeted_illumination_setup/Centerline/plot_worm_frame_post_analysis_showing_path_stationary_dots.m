function [] = plot_worm_frame_post_analysis_showing_path_stationary_dots(I, center_lines, centerline_color, centerline_stimulus, debugimage,in_track_index,current_ellipse_ratio,smoothed_path_x,smoothed_path_y,stimulus_delivered,path_history,demo_mode,dot_path_x,dot_path_y,dot_path,current_head_angle)
...
% % % % %     dot_path=[dot_path_x dot_path_y];
    ...
    %%%% trying to save longer path history
    if in_track_index>path_history*3
        path_t_long = smoothed_path(in_track_index-path_history*3:in_track_index,:) - smoothed_path(in_track_index,:)+centerline_correction;
    else
        path_t_long = smoothed_path(1:in_track_index,:) - smoothed_path(in_track_index,:)+centerline_correction;
    end
    
    %%%% plot the previous few DOTS of path or else plot from the beginning
% % % %     if in_track_index>path_history && rem(in_track_index,30)==0
% % % %         dot_path_t = smoothed_path(in_track_index,:) - smoothed_path(in_track_index,:)+centerline_correction;
% % % %         dot_path=[dot_path;dot_path_t];
% % % %     else in_track_index<=path_history && rem(in_track_index,30)==0;
% % % %         dot_path_t = smoothed_path(1:in_track_index,:) - smoothed_path(in_track_index,:)+centerline_correction;
% % % %         dot_path=[dot_path;dot_path_t];
% % % %     end
    
% % % % %     %%%% plot the previous few DOTS of path or else plot from the beginning
% % % % %     if in_track_index>path_history
% % % % %         dot_path_t = dot_path(in_track_index-path_history:in_track_index,:) - dot_path(in_track_index,:)+centerline_correction;
% % % % %     else
% % % % %         dot_path_t = dot_path(1:in_track_index,:) - dot_path(in_track_index,:)+centerline_correction;
% % % % %     end
...
% % % %     plot(dot_path(:,1),dot_path(:,2),'.', 'MarkerSize',18,'color',[1 1 0],'LineWidth',2);
...
%     title (sprintf('Frame=%d, E.R.=%.1f',in_track_index,current_ellipse_ratio));
    ...
    text(60, 60, num2str(current_head_angle,'%4.2f'), 'Color', 'y','FontSize',14);
