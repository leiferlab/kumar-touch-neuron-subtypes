function [] = plot_worm_frame_post_analysis_showing_path_mochi_data_2(I, center_lines, centerline_color, centerline_stimulus, debugimage,in_track_index,current_ellipse_ratio,smoothed_path_x,smoothed_path_y,stimulus_delivered,path_history,demo_mode,all_velocity,current_rails_dur,initial_frame)
...
% % %     plot([-round(size(all_velocity,1)./2)-1:1:round(size(all_velocity,1)./2)],all_velocity,'-r','Linewidth',2)
    ...
    if in_track_index>=initial_frame && in_track_index-initial_frame<=current_rails_dur
        light_pulse = area([initial_frame initial_frame+(in_track_index-initial_frame)], [7 7],'BaseValue',-0.3,'ShowBaseLine','off','FaceColor',[1 0 0],'LineStyle','none');
        alpha(0.2)
    hold on
    end
    
    if in_track_index>initial_frame+current_rails_dur
        light_pulse = area([initial_frame (initial_frame+current_rails_dur)], [7 7],'BaseValue',-0.3,'ShowBaseLine','off','FaceColor',[1 0 0],'LineStyle','none');
        alpha(0.2)
        hold on
    end
