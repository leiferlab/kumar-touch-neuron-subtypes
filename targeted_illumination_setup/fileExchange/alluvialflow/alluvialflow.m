...
%    parameters etc.
    ...
    % check if data is only one row, if so, pad it so that this still runs
    ...
    for k_left = 1:row_count % for each row
        ...
        patch(X, Y, patch_colors, 'FaceAlpha', 1, 'EdgeColor', 'none');
    end % for each row
    
    % Place left labels
    text(zeros(1, size(data, 1)) - 0.01, ...
         left_category_points + left_category_sizes./2, ...
         left_labels, 'FontSize', 12, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
     
    % Place right labels
    text(ones(1, size(data, 2)) + 0.01, ...
         right_category_points + right_category_sizes./2, ...
         right_labels, 'FontSize', 12, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle');
