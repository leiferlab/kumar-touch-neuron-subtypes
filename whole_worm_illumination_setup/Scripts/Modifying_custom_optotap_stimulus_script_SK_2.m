...
cd ('/projects/LEIFER/Sandeep/Data/20211013_array_light_mix_tap_duration_details_Sandeep_AML257_TsChR_ret_3sec')
...
analysis_new_data=1;
current_rails_dur=14*0.5;  %%% 14 fps x time in sec.
...
% % % %        
    ...
    LEDVoltages=LEDVoltages(1:3360);
...
for stimulus_index = 1:n_sti
    ...
    my_colors = behavior_colors;
    figure
    light_pulse = area([0 current_rails_dur/fps], [1 1],'facecolor',[0 0 1],'LineStyle','none');
    hold on;grid off
    for behavior_index = 1:number_of_behaviors
...
%     title(['Strain: F; Stimulus Intensity = ', num2str(stimulus_intensities(stimulus_index)),'\muW/mm^2' ' (n = ', num2str(n_tracks(stimulus_index)), ' tracks)']);
    title(['ZX15857; HisCl', ' (n = ', num2str(n_tracks(stimulus_index)), ' tracks)']);
%     tap_line=xline(0,'k--');
