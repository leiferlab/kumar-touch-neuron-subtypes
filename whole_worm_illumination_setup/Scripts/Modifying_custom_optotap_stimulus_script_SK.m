...
cd ('D:\Expt_Data\AML67_data\20200724_debugging_opto_response')
...
% % % %        
    ...
    if read_from_text_file==1
        %%%%%% determining light_before_tap by reading it from .txt file
        all_light_duration=unique(str2num(tap_parameters.textdata{2,8}));
        light_stim_after_tap=str2num(tap_parameters.textdata{2,3});
        light_before_tap=all_light_duration-light_stim_after_tap; %%% use this if you want to read this variable from .txt file
        else
        %%%%%% providing the value for light_before_tap by user
        light_before_tap=LED_before_tap; %%%% use this if you do not want to read this variable from .txt file
    end
    ...
    for m=1:size(light_before_tap,2)
        peak_locations_time_diff= LED_peaks(cluster_assignments{:,m});    %%%%% use this for LED_and_tap
...
for stimulus_index = 1:n_sti
    ...
    my_colors = behavior_colors;
    figure
    light_pulse = area([0 1], [1 1],'facecolor',[1 0 0],'LineStyle','none');
    hold on;
    for behavior_index = 1:number_of_behaviors
    ...
    title(['Stimulus Intensity = ', num2str(stimulus_intensities(stimulus_index)),'\muW/mm^2' ' (n = ', num2str(n_tracks(stimulus_index)), ' events)']);
...
for behavior_index = [8] %forward3; forward 5; fast reverse; turns
    ...
    for stimulus_index = 1:n_sti
        ...
        plot(-time_window_before/fps:1/fps:time_window_after/fps, squeeze(behavior_ratios_for_frame(behavior_index,stimulus_index,:)), '-', 'color', my_colors(stimulus_index,:),'Linewidth', 3,...
            'DisplayName',[num2str(stimulus_intensities(stimulus_index)), 'uW/mm^2 (n = ', num2str(n_tracks(stimulus_index)),')']);
    end
    hold off
    xlabel('Time (s)') % x-axis label
    ylabel('Behavioral Ratio') % y-axis label
